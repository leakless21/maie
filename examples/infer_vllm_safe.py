...
def main() -> int:
    ...
    input_group.add_argument(
        "--text", help="Direct text input (will be read from stdin if not provided)"
    )
    ...
    parser.add_argument(
        "--max-tokens",
        type=int,
        help="Override max tokens (auto-calculated if not set)",
    )
    parser.add_argument(
    ...
    if args.max_tokens is None:
        ...
        if args.task == "summary":
            ...
            auto_max_tokens = max(
                512, int(estimated_input_tokens * args.auto_max_tokens_ratio)
            )
