#!/usr/bin/env python3
"""
CLI: Speaker diarization inference with batch size testing and memory monitoring.

Standalone testing tool for diarization with configurable batch sizes to test 
memory management and performance tradeoffs.

Usage examples:
  # Basic diarization with ASR backend (Whisper or ChunkFormer)
  python examples/infer_diarization_test.py --audio data/audio/sample.wav --backend whisper

  # Test with custom batch sizes
  python examples/infer_diarization_test.py --audio data/audio/sample.wav \\
...
Output: Diarized segments with speaker attribution, timing info, and optional memory stats.
"""
...
from src.config.model import DiarizationSettings  # noqa: E402
...
def format_diarization_output(
    segments: List[Dict[str, Any]], json_format: bool = False
) -> str:
    """Format diarization results for display."""
    if json_format:
...
def run_diarization_test(
    ...
    try:
        ...
        logger.info("Transcribing audio with {} backend".format(backend))
        transcript = asr.transcribe(str(processing_audio_path))
        if not transcript:
        ...
        result = {
            ...
            "num_segments": len(diarized_segments or []),
        }
        ...
        result = {"success": False, "error": str(e), "audio_file": str(audio_path)}
        memory_stats = memory_monitor.stop() if memory_monitor else None
        return result, memory_stats
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Speaker diarization inference with batch size testing",
...
Examples:
  ...
  python examples/infer_diarization_test.py --audio data/audio/sample.wav \\
    ...
    parser.add_argument(
        "--audio", required=True, help="Path to input audio file (WAV, MP3, etc.)"
    )
    parser.add_argument(
        "--backend",
        choices=["whisper", "chunkformer"],
        default="whisper",
        help="ASR backend to use (default: whisper)",
    )
    parser.add_argument(
        "--embedding-batch-size",
        type=int,
        default=32,
        help="Batch size for speaker embedding extraction (default: 32, recommended by pyannote)",
    )
    parser.add_argument(
        "--segmentation-batch-size",
        type=int,
        default=32,
        help="Batch size for speech segmentation (default: 32, recommended by pyannote)",
    )
    parser.add_argument(
        "--monitor-memory",
        action="store_true",
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output results as JSON (useful for parsing/automation)",
    )
    parser.add_argument(
        "--test-batch-sizes",
        help='Test multiple batch size configurations (comma-separated, e.g. "8,16,32,64")',
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if args.test_batch_sizes:
        ...
        if args.json:
            ...
            for result in all_results:
                batch_size = result.get("batch_sizes", {}).get("embedding_batch_size")
    ...
    if args.json:
        ...
        if result.get("success"):
            ...
            if result["segments"]:
                print(format_diarization_output(result["segments"], json_format=False))
            else:
