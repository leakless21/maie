...
def main() -> int:
    ...
    parser.add_argument("--backend", default="chunkformer", help="ASR backend")
    ...
    try:
        ...
        transcript_tokens = len(
            llm.tokenizer.encode(transcript, add_special_tokens=False)
        )
        ...
        available_output = (
            max_model_len - prompt_tokens - 128
        )  # 128 token safety margin

        print("\n4. Token Budget Breakdown:")
        print(f"   Total context window:  {max_model_len:,} tokens")
        print(
            f"   Used by prompt:        {prompt_tokens:,} tokens ({(prompt_tokens / max_model_len) * 100:.1f}%)"
        )
        print("   Safety margin:         128 tokens")
        ...
        if available_output < recommended_output:
            ...
            print(
                f"   - Prompt uses {prompt_tokens:,} tokens, leaving only {available_output:,} for output"
            )
            print(
                f"   - Need at least {recommended_output:,} tokens for quality summary"
            )
            print("\n   SOLUTIONS:")
            print(
                f"   1. Increase max_model_len to at least {prompt_tokens + recommended_output + 128:,}"
            )
            print("   2. Use a shorter transcript or split into chunks")
            print("   3. Simplify the prompt template")
        else:
