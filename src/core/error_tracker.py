...
def log_stage_entry(stage_name: str, **kwargs) -> None:
    ...
    logger.info(
        "Entering stage: {stage} | Context: {context}", stage=stage_name, context=kwargs
    )
...
def log_stage_exit(stage_name: str, success: bool = True, **kwargs) -> None:
    ...
    if success:
        logger.info(
            "Exiting stage: {stage} | Success: {success} | Context: {context}",
            stage=stage_name,
            success=success,
            context=kwargs,
        )
...
def log_error_with_context(
    message: str, error: Optional[Exception] = None, **context
) -> None:
    """
    ...
    if error:
        ...
        logger.error(
            "Error: {message} | Context: {context}", message=message, context=context
        )
...
def log_preprocessing_error(
    error: Exception, input_file: str = "unknown", **context
) -> None:
    """Log preprocessing-specific errors."""
    log_error(
        error, context={"input_file": input_file, **context}, stage="preprocessing"
    )
