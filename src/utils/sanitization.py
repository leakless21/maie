...
def sanitize_metadata(metadata: Any, _seen: Optional[Set[int]] = None) -> Any:
    ...
    if isinstance(metadata, dict):
        ...
        for key, value in metadata.items():
            # Sanitize keys too
            clean_key = (
                sanitize_text(str(key), preserve_newlines=False)
                if key is not None
                else key
            )
        ...
        if hasattr(metadata, "model_dump") and callable(
            getattr(metadata, "model_dump")
        ):
            try:
...
def validate_mime_type(
    mime_type: str, allowed_mime_types: Optional[List[str]] = None
) -> bool:
...
def remove_control_characters(
    text: str, allowed_chars: Optional[List[str]] = None
) -> str:
