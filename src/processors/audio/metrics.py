...
class AudioMetricsCollector:
    ...
    def calculate_confidence(self, audio_path: str) -> float:
        ...
        try:
            ...
            confidence = min(1.0, float(signal_power) / 100.0)  # Normalize based on expected power
            confidence = min(confidence, float(signal_std) / 1000.0)  # Also consider standard deviation
    ...
    def validate_audio_properties(self, audio_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if mono
            is_mono = len(audio_data.shape) == 1 or audio_data.shape[1] == 1 if len(audio_data.shape) > 1 else True
