...
class MAIEProcessManager:
    ...
    def find_api_processes(self) -> List[psutil.Process]:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                elif "pixi" in cmdline and "run" in cmdline and "api" in cmdline:
                    processes.append(proc)
    ...
    def find_worker_processes(self) -> List[psutil.Process]:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                elif "pixi" in cmdline and "run" in cmdline and "worker" in cmdline:
                    processes.append(proc)
    ...
    def display_processes(self):
        ...
        if self.api_processes:
            for proc in self.api_processes:
                try:
                    ...
                    print(
                        f"  PID: {proc.pid:>6} | {proc.name():>12} | {cmdline[:80]}..."
                    )
                except (psutil.NoSuchProcess, psutil.AccessDenied):
        ...
        if self.redis_workers:
            for worker in self.redis_workers:
                print(
                    f"  Name: {worker['name']:>20} | State: {worker['state']:>10} | Last Heartbeat: {worker['last_heartbeat']}"
                )
        else:
    ...
    def interactive_kill(self):
        ...
        response = (
            input(f"\n🤔 Kill all {len(all_processes)} processes? [y/N]: ")
            .strip()
            .lower()
        )
...
Examples:
  ...
  python scripts/process_manager.py --interactive     # Interactive kill mode
    ...
    parser.add_argument("--kill", action="store_true", help="Kill all found processes")
    parser.add_argument(
        "--force",
        action="store_true",
