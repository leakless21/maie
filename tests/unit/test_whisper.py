...
def test_execute_returns_asr_result(inject_mock_faster_whisper, temp_audio_file, tmp_path):
...
def test_config_compute_type_integration(inject_mock_faster_whisper, mock_config, tmp_path):
...
def test_config_beam_size_integration(inject_mock_faster_whisper, mock_config, tmp_path):
...
def test_config_condition_on_previous_text(inject_mock_faster_whisper, mock_config, tmp_path):
...
def test_vad_parameters_preparation(inject_mock_faster_whisper, mock_config, temp_audio_file, tmp_path):
...
# ============================================================================
# Test Section 7: CPU Fallback Mechanism
# ============================================================================

def test_cpu_fallback_enabled(inject_mock_faster_whisper, mock_config, tmp_path):
    """Test that CPU fallback is configurable."""
    from src.processors.asr.whisper import WhisperBackend
    
    model_path = tmp_path / "fake_model"
    model_path.mkdir()
    
    mock_config.set("whisper_cpu_fallback", True)
    mock_config.set("whisper_device", "cuda:999")  # Invalid device
    mock_config.set("whisper_model_path", str(model_path))
    
    backend = WhisperBackend()
    # In real code, this would fallback to CPU
...
def test_cpu_fallback_disabled(inject_mock_faster_whisper, mock_config, tmp_path):
    """Test behavior when CPU fallback is disabled."""
    from src.processors.asr.whisper import WhisperBackend
    
    model_path = tmp_path / "fake_model"
    model_path.mkdir()
    
    mock_config.set("whisper_cpu_fallback", False)
    mock_config.set("whisper_model_path", str(model_path))
...
def test_prepare_transcribe_kwargs(inject_mock_faster_whisper, mock_config, temp_audio_file, tmp_path):
...
def test_vad_parameters_in_kwargs(inject_mock_faster_whisper, mock_config, temp_audio_file, tmp_path):
...
def test_language_parameter_in_kwargs(inject_mock_faster_whisper, mock_config, temp_audio_file, tmp_path):
