...
def test_load_model_invalid_path_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        ChunkFormerBackend,
        "_load_model",
        lambda self, **k: (_ for _ in ()).throw(RuntimeError("failed")),
        raising=True,
    )
...
def test_execute_calls_model_decode(monkeypatch):
    ...
    class FakeModel:
        ...
        def decode(self, audio_chunks, left_context=None, right_context=None, **kwargs):
            calls.decode += 1
            return {
                "segments": [{"start": 0.0, "end": 1.0, "text": "hello chunk"}],
                "language": "en",
                "confidence": 0.9,
            }
...
def test_execute_without_model_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        ChunkFormerBackend, "_load_model", lambda self, **k: None, raising=True
    )
...
def test_execute_performance_smoke(monkeypatch):
    ...
    class FastModel:
        ...
        def decode(self, audio_chunks, **kwargs):
            return {
                "segments": [{"start": 0.0, "end": 0.5, "text": "fast"}],
                "language": "en",
                "confidence": 0.9,
            }
