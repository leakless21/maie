...
class TestAudioPreprocessor:
    ...
    def test_probe_parses_ffprobe_output(self, tmp_path, monkeypatch):
        ...
        probe_out = {
            "format": {"duration": "2.34", "format_name": "ogg"},
            "streams": [
                {
                    "codec_type": "audio",
                    "sample_rate": "48000",
        ...
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: fake_subprocess_result(0, stdout=json.dumps(probe_out)),
        )
    ...
    def test_preprocess_normalizes_when_needed(self, tmp_path, monkeypatch):
        ...
        probe = {
            "format": {"duration": "1.5", "format_name": "mp3"},
            "streams": [{"codec_type": "audio", "sample_rate": "44100", "channels": 2}],
        }
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: fake_subprocess_result(0, stdout=json.dumps(probe)),
        )
        # stub normalize to return expected path
        out = tmp_path / "preprocessed.wav"
        monkeypatch.setattr(
            preprocessor_module.AudioPreprocessor,
            "_normalize_audio",
            lambda self, inp, meta: out,
        )
    ...
    def test_preprocess_rejects_too_short(self, tmp_path, monkeypatch):
        ...
        probe_out = {
            "format": {"duration": "0.5", "format_name": "wav"},
    ...
    def test_normalize_failure_raises(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            subprocess,
            "run",
            lambda *a, **k: fake_subprocess_result(1, stderr="ffmpeg error"),
        )
        with pytest.raises(ValueError):
            p._normalize_audio(
                src,
    ...
    def test_integration_calls_factory_with_preprocessed_path(
