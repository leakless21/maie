...
from src.processors.llm.schema_validator import load_template_schema, validate_llm_output
...
def test_text_enhancement_template():
    ...
    try:
        ...
        print(f"   ✓ Original text length: {len(validated_data['original_text'])} chars")
        print(f"   ✓ Enhanced text length: {len(validated_data['enhanced_text'])} chars")
    ...
    correction_types = schema['properties']['corrections']['items']['properties']['type']['enum']
