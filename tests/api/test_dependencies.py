...
class TestGuardSignature:
    ...
    async def test_api_key_guard_signature_accepts_connection_and_handler(self):
        ...
        assert len(params) == 2, f"Guard should have 2 parameters, got {len(params)}: {params}"
...
class TestTimingSafeAPIKeyComparison:
    ...
    async def test_guard_uses_hmac_compare_digest(self):
        ...
        assert 'hmac.compare_digest' in source, "Guard must use hmac.compare_digest for timing-safe comparison"
    ...
    async def test_timing_attack_resistance(self):
        ...
        with patch('src.api.dependencies.settings.secret_api_key', correct_key):
            ...
            ratio = max(time_early, time_late) / min(time_early, time_late) if min(time_early, time_late) > 0 else 1.0
            
            # Allow 10x variance (constant-time should be much closer, but allow for system noise)
            assert ratio < 10, f"Timing difference too large: {ratio}x (suggests non-constant-time comparison)"
    ...
    async def test_guard_validates_key_length_before_comparison(self):
        ...
        with patch('src.api.dependencies.settings.secret_api_key', 's' * 64):
            ...
            assert "format" in str(exc_info.value).lower() or "length" in str(exc_info.value).lower()
...
class TestRequestValidation:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(reason="Template validation not implemented - available_templates field doesn't exist in Settings yet")
...
class TestRedisDependencies:
    ...
    async def test_get_redis_client_returns_async_redis(self):
        ...
        # Verify type
        assert isinstance(client, AsyncRedis), f"Expected AsyncRedis, got {type(client)}"
    ...
    def test_get_rq_queue_returns_queue_with_sync_redis(self):
        ...
        assert isinstance(queue.connection, SyncRedis), "Queue must use synchronous Redis client"
    ...
    def test_get_rq_queue_with_custom_name(self):
        ...
        for name in custom_names:
            ...
            assert queue.name == name, f"Expected queue name '{name}', got '{queue.name}'"
...
class TestRedisConnectionPooling:
    ...
    async def test_results_redis_has_custom_timeout(self):
        ...
        assert 'socket_timeout' in conn_kwargs, \
            "socket_timeout must be configured"
    ...
    async def test_redis_pool_uses_from_url(self):
        ...
        from src.api.dependencies import get_redis_client, get_results_redis, get_sync_redis
        import inspect
