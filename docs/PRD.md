...
- FR-2: ASR Backend — The system provides a single ASR backend for V1.0:
  - whisper: Default backend for V1.0, using Whisper-based models via CTranslate2 runtime. The default model variant is EraX-WoW-Turbo V1.1 (`erax-wow-turbo`), which provides native punctuation and capitalization, optimized for throughput with VAD filtering. Alternative backends are deferred to a future release.

- FR-3: Text Enhancement — Text enhancement is an optional pipeline step. It MUST be bypassed when the selected ASR backend provides adequate punctuation and casing (e.g., `whisper` with `erax-wow-turbo` variant). When required (with backends that lack punctuation; not applicable to V1.0 default), the pipeline will use the LLM for punctuation and capitalization correction.
...
- FR-8: Discovery Endpoints — The API MUST expose discovery endpoints to allow developers to query for available resources.
  - `GET /v1/models`: Returns a list of available ASR backends and model variants. In V1.0, this returns only the Whisper backend with its default variant and metadata.
...
- (removed) `asr_backend`: Backend selection is fixed to `"whisper"` in V1.0 to reduce complexity.
